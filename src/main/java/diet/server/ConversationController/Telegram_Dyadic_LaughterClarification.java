 ...
 * @author LX1C
 */
public class Telegram_Dyadic_LaughterClarification extends TelegramController{
   ...
   // Participant pMatcher;
    ...
    public Telegram_Dyadic_LaughterClarification(Conversation c) {
        super(c);
        Thread t = new Thread(){public void run(){loop();}};
        t.start();
    }

    public Telegram_Dyadic_LaughterClarification(Conversation c, long istypingtimeout) {
    ...
    String[] whyvariants = new String[]{"what's funny?", "What's funny?"};
    ...
    public String generateQuery(String target) {
        ...
        return this.whyvariants[r.nextInt(this.whyvariants.length)];
    }
    ...
    public synchronized void generateClarification(TelegramParticipant sender, String t){
         ...
         for(int i =0; i < possibleTargets.size(); i++){
             ...
             this.currentTarget = (String)possibleTargets.elementAt(i);
             ...
             try{
                 ...
                 if(mode==this.waitingaftertargetbeforesending){
                      ...
                      if(timeSinceDetection>this.durationToWaitAfterTargetBeforeIntervention){
                          generateQuery(currentTarget);
                          Vector<String> vectorWhyVariants = new Vector<String>(Arrays.asList(whyvariants));
                          CyclicRandomTextGenerator crt = new CyclicRandomTextGenerator( vectorWhyVariants);
